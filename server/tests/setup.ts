...
beforeEach(async () => {
  ...
  if (!dbReady) return;
  await pool.query(
    'TRUNCATE ai_call_logs, enrichment_job_leads, enrichment_jobs, lead_stage_transitions, campaign_recipients, campaigns, deal_activities, deals, message_templates, messages, conversations, leads, sessions, auth_tokens, users, whatsapp_instance RESTART IDENTITY CASCADE',
  );
});
